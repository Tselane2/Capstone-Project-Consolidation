...
security settings, installed apps, middleware, templates, database connections,
authentication, REST framework settings, email configuration, and third‑party
API credentials.

Environment variables are loaded using `python-decouple` to keep sensitive
information out of source control. The project defaults to SQLite for local
development but supports PostgreSQL or MariaDB/MySQL when corresponding
environment variables are provided.

These settings are used by Django to initialize and run the application.
"""
